...
public partial class search : System.Web.UI.Page
{
    ...
    public DataTable GetDataTable()
    {
        ...
        string strqry = "";
        string strqrynew = "";
        strqry = "select distinct pageid,pagetitle,  LTRIM(PageName) stitle, PageMeta as sdesc,PageMetaDesc as ldesc,pageurl=case when rewriteurl<>'' then rewriteurl Else replace(pageurl,'~/','')   End from pagemaster where PageStatus=1 and collageid=1 ";
        strqrynew = "select distinct pageid,pagetitle, PageName stitle, PageMeta as sdesc,PageMetaDesc as ldesc,pageurl=case when rewriteurl<>'' then rewriteurl Else replace(pageurl,'~/','')   End from pagemaster where PageStatus=1 and collageid=1 ";
        ...
        Parameters.Clear();
        strqry = "";
        strqry = "select distinct facultyid as pageid,fname as stitle, Designation as Designation, smalldesc as ldesc, pageurl='facultydetail.aspx?mpgid=77&pgidtrail=147&facultyid='+convert(varchar,facultyid) from Addfacultymaster where status=1";
        ...
        strqry = "select distinct eventsid as pageid,eventstitle as stitle,shortdesc as ldesc, pageurl='newsdetail.aspx?mpgid=159&pgidtrail=160&eventsid='+convert(varchar,eventsid) from events where status=1 and ntypeid in(1,2)";
        ...
        if (!string.IsNullOrEmpty(Request.QueryString["search"]))
        {
            ...
            Parameters.Add("@PageMeta", Server.UrlDecode(Request.QueryString["search"]));
            Parameters.Add("@PageTitle", Server.UrlDecode(Request.QueryString["search"]));
            Parameters.Add("@NewsTitle", Server.UrlDecode(Request.QueryString["search"]));

            strqry += " and (PageMetaDesc like '%'+@PageMetaDesc+'%' or PageMeta like '%'+@PageMeta+'%' or pagetitle like '%'+@pagetitle+'%' or eventstitle like '%'+@NewsTitle+'%')";
        }
