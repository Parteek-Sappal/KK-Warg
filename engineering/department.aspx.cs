...
public partial class engineering_department : System.Web.UI.Page
{
    ...
    private void binddata()
    {
        ...
        if (!string.IsNullOrEmpty(litRecruiters.Text))
        {
            panelrecruiter.Visible = true;
        }
        parameters.Clear();
        parameters.Add("@deptid", Conversion.Val(Request.QueryString["deptid"]));
        ...
        parameters.Clear();
        parameters.Add("@deptid", Conversion.Val(Request.QueryString["deptid"]));
        clsm.repeaterDatashow_Parameter(rptfaculty, "select top 4 afm.*,fdesignation.designation as designationname from Addfacultymaster afm inner join Facultydesignation fdesignation on fdesignation.fdid=afm.Designation inner join map_institute_department_faculty map on map.facultyid=afm.facultyid and map.deptid=@deptid where afm.status=1 and fdesignation.status=1 and map.showonhome=1 order by afm.displayorder", parameters);
    ...
    protected void rptgallery_OnItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            if (Conversion.Val(littypeid.Text) == 1)
            {
                ...
                ank.HRef = "/engineering/mediadetail.aspx?mpgid=31&pgid1=32&pgidtrail=38&deptid=" + Conversion.Val(Request.QueryString["deptid"]) + "&albumid=" + Conversion.Val(litalbumid.Text);
            }
            else if (Conversion.Val(littypeid.Text) == 2)
            {
                ...
                ank.HRef = "/engineering/videodetail.aspx?mpgid=31&pgid1=32&pgidtrail=38&deptid=" + Conversion.Val(Request.QueryString["deptid"]) + "&albumid=" + Conversion.Val(litalbumid.Text);
            }
            if (cnt == 0)
            {
    ...
    private void LinkDetail(double ntypeid, double eventsid, string eventstitle, HtmlAnchor a)
    {
        if (ntypeid == 1)
        {
            //a.HRef = "/engineering/department/news-detail/" + clsm.replacestring(eventstitle) + "/" + Conversion.Val(eventsid) + "/" + deptid;

            a.HRef = "/engineering/newsdetail.aspx?mpgid=32&pgidtrail=133&deptid=" + Conversion.Val(deptid) + "&eventsid=" + Conversion.Val(eventsid) + "";
        }
        else if (ntypeid == 2)
        {
            //a.HRef = "/engineering/department/events-detail/" + clsm.replacestring(eventstitle) + "/" + Conversion.Val(eventsid) + "/" + deptid;

            a.HRef = "/engineering/newsdetail.aspx?mpgid=32&pgidtrail=34&deptid=" + Conversion.Val(deptid) + "&eventsid=" + Conversion.Val(eventsid) + "";
