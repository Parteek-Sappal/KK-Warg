...
public partial class engineering_mediadetail : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Conversion.Val(Request.QueryString["albumid"]) > 0)
        {
            ...
            parameters.Clear();
            parameters.Add("@albumid", Conversion.Val(Request.QueryString["albumid"]));
            clsm.repeaterDatashow_Parameter(rptalbumdetail, "select photoid,albumid,phototitle,uploadphoto from albumphoto where albumid=@albumid and status=1", parameters);
        }
    }
}