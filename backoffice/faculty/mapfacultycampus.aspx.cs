...
public partial class backoffice_faculty_mapfacultycampus : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        foreach (DataListItem item in collegelist.Items)
        {
            ...
            {
                if (clsm.Checking("select * from map_faculty_campus  where facultyid='" + Conversion.Val(Request.QueryString["facultyid"]) + "' and campusid= '" + Conversion.Val(lblcampusid.Text) + "' ") == false)
                {
                    if (clsm.Checking("select mapid from map_faculty_campus where campusid='"
                                    + (Conversion.Val(lblcampusid.Text) + "' and facultyid='"
                                    + (Conversion.Val(Request.QueryString["facultyid"])) + "'")) == false)
                    {
                        clsm.ExecuteQry("insert into map_faculty_campus (facultyid,campusid)values("
            ...
            {
                clsm.ExecuteQry("delete from map_faculty_campus where campusid="
                                + (Conversion.Val(lblcampusid.Text) + " and facultyid="
