    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        foreach (DataListItem item in researchlist.Items)
        {
            ...
            if (checkfeature.Checked == true)
            {
                ...
                {
                    if (clsm.Checking("select mfid from map_research_faculty where researchid='"
                                    + (Conversion.Val(lblEventsid.Text) + "' and facultyid='"
                                    + (Conversion.Val(Request.QueryString["facultyid"])) + "'")) == false)
                    {
                        clsm.ExecuteQry("insert into map_research_faculty (facultyid,researchid,displayorder)values("
                ...
                {
                    clsm.ExecuteQry("update  map_research_faculty  set  displayorder=" + Conversion.Val(txtdisplayorder.Text) + " where facultyid=" + (Conversion.Val(Request.QueryString["facultyid"])) + " and researchid=" + Conversion.Val(lblEventsid.Text) + "");
                }
            }
            else
            {

                clsm.ExecuteQry("delete from map_research_faculty where researchid="
                                + (Conversion.Val(lblEventsid.Text) + " and facultyid="
                                + (Conversion.Val(Request.QueryString["facultyid"]) + "  ")));
            }
