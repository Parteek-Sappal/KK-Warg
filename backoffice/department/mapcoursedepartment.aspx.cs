...
public partial class backoffice_department_mapcoursedepartment : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        foreach (DataListItem item in courselist.Items)
        {
            ...
            {
                if (clsm.Checking("select * from map_course_department  where deptid='" + Conversion.Val(Request.QueryString["deptid"]) + "' and courseid= '" + Conversion.Val(lblcourseid.Text) + "' ") == false)
                {
                    if (clsm.Checking("select mapid from map_course_department where courseid='"
                                    + (Conversion.Val(lblcourseid.Text) + "' and deptid='"
                                    + (Conversion.Val(Request.QueryString["deptid"])) + "'")) == false)
                    {
                        clsm.ExecuteQry("insert into map_course_department (deptid,courseid)values("
                                      + (Request.QueryString["deptid"]) + ","
                                      + (Conversion.Val(lblcourseid.Text) + ")"));
                    }
                }
            }
            else
            {
                clsm.ExecuteQry("delete from map_course_department where courseid="
                                + (Conversion.Val(lblcourseid.Text) + " and deptid="
                                + (Conversion.Val(Request.QueryString["deptid"]) + "  ")));
            }
