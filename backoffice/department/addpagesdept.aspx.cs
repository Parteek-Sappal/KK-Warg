    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (String.IsNullOrEmpty(Pageid.Text))
            {
                ...
                clsm.ExecuteQry("update PageMasterdept set pageurl='" + purl + "' where pageid=" + Convert.ToString(var) + "");
                // '' end page url
                ...
                clsm.ExecuteQry("update PageMasterdept set rewriteid='" + Strrewriteid + "' where pageid=" + Convert.ToString(var) + "");
                if (!string.IsNullOrEmpty(Path.GetFileName(File1.PostedFile.FileName)))
                {
                    Parameters.Clear();
