   ...
   mainclass clsm = new mainclass();
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        foreach (DataListItem item in locationlist.Items)
        {
            ...
            {
                if (clsm.Checking("select * from map_course_eligibility  where eid='" + Conversion.Val(Request.QueryString["eid"]) + "' and courseid= '" + Conversion.Val(lblcentresid.Text) + "' ") == false)
                {
                    if (clsm.Checking("select mapid from map_course_eligibility where courseid='"
                                    + (Conversion.Val(lblcentresid.Text) + "' and eid='"
                                    + (Conversion.Val(Request.QueryString["eid"])) + "'")) == false)
                    {
                        clsm.ExecuteQry("insert into map_course_eligibility (eid,courseid)values("
                                      + (Request.QueryString["eid"]) + ","
                                      + (Conversion.Val(lblcentresid.Text) + ")"));
            ...
            else
            {
                ...
                clsm.ExecuteQry("delete from map_course_eligibility where courseid="
                                + (Conversion.Val(lblcentresid.Text) + " and eid="
                                + (Conversion.Val(Request.QueryString["eid"]) + "  ")));
            }
