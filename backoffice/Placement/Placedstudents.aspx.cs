    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(spid.Text))
            {
                ...
                //}
                CKeditor2.ReadOnly = true;
                string var = clsm.MasterSave(this, spid.Parent, 20, mainclass.Mode.modeAdd, "PlacedstudentSP", Session["UserId"].ToString()).ToString();
                CKeditor2.ReadOnly = false;
                if (!string.IsNullOrEmpty(File1.PostedFile.FileName))
                {
                    ...
                    if (F1.Exists)
                    {
                        //clsm.ExecuteQry("delete from award_achive where aaid=" & var & "")
                        Parameters.Clear();
                        Parameters.Add("@spid", var);
            ...
            {
                CKeditor2.ReadOnly = true;
                string var = clsm.MasterSave(this, spid.Parent, 20, mainclass.Mode.modeModify, "PlacedstudentSP", Session["UserId"].ToString()).ToString();
                CKeditor2.ReadOnly = false;
                //if (!string.IsNullOrEmpty(File1.PostedFile.FileName))
                //{
                //    Parameters.Clear();
