...
public partial class backoffice_Fee_map_course_hostelfee : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        foreach (DataListItem item in locationlist.Items)
        {
            ...
            {
                if (clsm.Checking("select * from map_course_hostelfee  where cfid='" + Conversion.Val(Request.QueryString["eid"]) + "' and courseid= '" + Conversion.Val(lblcourseid.Text) + "' and collageid= '" + Conversion.Val(lblcollageid.Text) + "' ") == false)
                {
                    if (clsm.Checking("select hostelfeeid from map_course_hostelfee where collageid=" + Conversion.Val(lblcollageid.Text) + " and courseid='"
                                    + (Conversion.Val(lblcourseid.Text) + "' and cfid='"
                                    + (Conversion.Val(Request.QueryString["cfid"])) + "'")) == false)
                    {
                        clsm.ExecuteQry("insert into map_course_hostelfee (cfid,courseid,collageid)values(" + Conversion.Val(Request.QueryString["cfid"]) + "," + Conversion.Val(lblcourseid.Text) + "," + Conversion.Val(lblcollageid.Text) + ")");
            ...
            else
            {
                ...
                clsm.ExecuteQry("delete from map_course_hostelfee where  cfid=" + Conversion.Val(Request.QueryString["cfid"]) + " and courseid=" + Conversion.Val(lblcourseid.Text) + " and collageid=" + Conversion.Val(lblcollageid.Text) + "");
