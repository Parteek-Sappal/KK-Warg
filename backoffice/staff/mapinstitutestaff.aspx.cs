...
public partial class backoffice_staff_mapinstitutestaff : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        foreach (DataListItem item in collegelist.Items)
        {
            ...
            {
                if (clsm.Checking("select * from map_staff_institute  where staffid=" + Conversion.Val(Request.QueryString["staffid"]) + " and collageid=" + Conversion.Val(lblcollageid.Text) + " ") == false)
                {
                    if (clsm.Checking("select mapid from map_staff_institute where collageid='"
                                    + (Conversion.Val(lblcollageid.Text) + "' and staffid='"
                                    + (Conversion.Val(Request.QueryString["staffid"])) + "'")) == false)
                    {
                        clsm.ExecuteQry("insert into map_staff_institute (staffid,collageid)values("
            ...
            {
                clsm.ExecuteQry("delete from map_staff_institute where collageid="
                                + (Conversion.Val(lblcollageid.Text) + " and staffid="
                                + (Conversion.Val(Request.QueryString["staffid"]) + "  ")));
            }
