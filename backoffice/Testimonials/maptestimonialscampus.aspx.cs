...
public partial class backoffice_Testimonials_maptestimonialscampus : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        foreach (DataListItem item in collegelist.Items)
        {
            ...
            {
                if (clsm.Checking("select * from map_testimonials_campus  where testimonialid='" + Conversion.Val(Request.QueryString["testimonialid"]) + "' and campusid= '" + Conversion.Val(lblcampusid.Text) + "' ") == false)
                {
                    if (clsm.Checking("select mapid from map_testimonials_campus where campusid='"
                                    + (Conversion.Val(lblcampusid.Text) + "' and testimonialid='"
                                    + (Conversion.Val(Request.QueryString["testimonialid"])) + "'")) == false)
                    {
                        clsm.ExecuteQry("insert into map_testimonials_campus (testimonialid,campusid)values("
            ...
            {
                clsm.ExecuteQry("delete from map_testimonials_campus where campusid="
                                + (Conversion.Val(lblcampusid.Text) + " and testimonialid="
                                + (Conversion.Val(Request.QueryString["testimonialid"]) + "  ")));
            }
