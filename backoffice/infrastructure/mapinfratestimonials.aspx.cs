...
public partial class backoffice_infrastructure_mapinfratestimonials : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        foreach (DataListItem item in testimoniallist.Items)
        {
            ...
            {
                if (clsm.Checking("select * from map_infra_testimonials  where infid='" + Conversion.Val(Request.QueryString["infid"]) + "' and testimonialid= '" + Conversion.Val(lbltestimonialid.Text) + "' ") == false)
                {
                    if (clsm.Checking("select mapid from map_infra_testimonials where testimonialid='"
                                    + (Conversion.Val(lbltestimonialid.Text) + "' and infid='"
                                    + (Conversion.Val(Request.QueryString["infid"])) + "'")) == false)
                    {
                        clsm.ExecuteQry("insert into map_infra_testimonials (infid,testimonialid)values("
            ...
            else
            {
                ...
                clsm.ExecuteQry("delete from map_infra_testimonials where testimonialid="
                                + (Conversion.Val(lbltestimonialid.Text) + " and infid="
                                + (Conversion.Val(Request.QueryString["infid"]) + "  ")));
            }
