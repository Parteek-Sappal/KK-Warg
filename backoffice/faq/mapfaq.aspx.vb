...
Partial Class backoffice_faq_mapfaq
    ...
    Sub map_cat_product()
        For Each row1 As DataListItem In dl_sgroup.Items
            ...
            If checkfeature.Checked = True Then
                If clsm.Checking(" select mcid from map_faq_course where  faqid=" & Val(Request.QueryString("faqid")) & "  and courseid=" & Val(lblcourseid.Text) & "") = False Then

                    clsm.ExecuteQry("insert into map_faq_course (subdeptid,courseid,faqid) values(" & Val(0) & "," & Val(lblcourseid.Text) & "," & Val(Request.QueryString("faqid")) & ")")

                End If
            Else
                clsm.ExecuteQry("delete from map_faq_course where faqid=" & Val(Request.QueryString("faqid")) & " and  courseid=" & Val(lblcourseid.Text) & "  ")
            End If
    ...
    'Sub map_faq_depart()
    '    For Each row2 As DataListItem In dl_depart.Items
    '        Dim lblsubdeptid As Label = row2.FindControl("lblsubdeptid")
    '        Dim checkfeaturedep As CheckBox = row2.FindControl("checkfeaturedep")
    '        If checkfeaturedep.Checked = True Then
    '            If clsm.Checking(" select mcid from map_faq_department where  faqid=" & Val(Request.QueryString("faqid")) & "  and subdeptid=" & Val(lblsubdeptid.Text) & "") = False Then

    '                clsm.ExecuteQry("insert into map_faq_department (subdeptid,courseid,faqid) values(" & Val(lblsubdeptid.Text) & "," & Val(0) & "," & Val(Request.QueryString("faqid")) & ")")
    ...
    '            End If
    '        Else
    '            clsm.ExecuteQry("delete from map_faq_department where faqid=" & Val(Request.QueryString("faqid")) & " and  subdeptid=" & Val(lblsubdeptid.Text) & "  ")
    ...
    'Protected Sub streamid_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles streamid.SelectedIndexChanged
    '    fillgrid()
    '    'filldepartgrid()
    'End Sub
End Class
