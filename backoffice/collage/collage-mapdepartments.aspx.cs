...
public partial class backoffice_collage_collage_mapdepartments : System.Web.UI.Page
{
    ...
    void map_cat_product()
    {
        ...
        foreach (DataListItem row1 in dl_sgroup.Items)
        {
            ...
            {
                 if ((clsm.Checking((" select mdeptid from map_collage_departments where collageid="
                                + (double.Parse(Request.QueryString["clid"]) + (" and deptid="
                                + (double.Parse(lbldeptid.Text) + ""))))) == false))
                {
                    clsm.ExecuteQry(("insert into map_collage_departments (collageid,deptid) values("
                                    + (double.Parse(Request.QueryString["clid"]) + (","
                                    + (double.Parse(lbldeptid.Text) + ")")))));
                    flag1 = true;
                }
                else
                {
            ...
            {
                clsm.ExecuteQry(("delete from map_collage_departments where collageid="
                                + (double.Parse(Request.QueryString["clid"]) + (" and deptid="
                                + (double.Parse(lbldeptid.Text) + "  ")))));
            }
