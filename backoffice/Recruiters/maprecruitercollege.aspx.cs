...
public partial class backoffice_Recruiters_maprecruitercollege : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        foreach (DataListItem item in collegelist.Items)
        {
            ...
            {
                if (clsm.Checking("select * from map_recruiters_institute  where imgid='" + Conversion.Val(Request.QueryString["imgid"]) + "' and collageid= '" + Conversion.Val(lblcollageid.Text) + "' ") == false)
                {
                    if (clsm.Checking("select mapid from map_recruiters_institute where collageid='"
                                    + (Conversion.Val(lblcollageid.Text) + "' and imgid='"
                                    + (Conversion.Val(Request.QueryString["imgid"])) + "'")) == false)
                    {
                        clsm.ExecuteQry("insert into map_recruiters_institute (imgid,collageid)values("
            ...
            {
                clsm.ExecuteQry("delete from map_recruiters_institute where collageid="
                                + (Conversion.Val(lblcollageid.Text) + " and imgid="
                                + (Conversion.Val(Request.QueryString["imgid"]) + "  ")));
            }
